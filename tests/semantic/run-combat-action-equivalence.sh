...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/realmz-combat-semantic-test.XXXXXX")"
cleanup() {
  rm -rf -- "$tmp_dir"
}
...
(
  ...
  "$cxx" "${flags[@]}" \
    ...
    -o "$tmp_dir/CombatActionEquivalenceTest"
)

"$tmp_dir/CombatActionEquivalenceTest"
