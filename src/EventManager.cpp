...
struct PendingSemanticUseTorchSource {
  uint8_t member;
  uint8_t slot;
};

static std::optional<PendingSemanticUseTorchSource>
    pending_semantic_use_torch_source;

static void clear_pending_semantic_use_torch_source() noexcept {
  pending_semantic_use_torch_source.reset();
}

static void stage_semantic_use_torch_source(
    uint8_t member,
    uint8_t slot) noexcept {
  pending_semantic_use_torch_source = {.member = member, .slot = slot};
}
...
public:
  ...
  bool push_semantic_use_torch_event(uint32_t tagged_message) {
    if (!RealmzIsSemanticUseTorchTag(tagged_message)) {
      return false;
    }
    // Torch has no Classic key binding. Keep the exact source locator tagged
    // until the guarded world consumer can recapture it and stage one neutral,
    // one-shot app-event handoff into the real Classic Torch control.
    auto& ev = this->event_queue.emplace_back();
    ev.what = app1Evt;
    ev.message = tagged_message;
    ev.when = TickCount();
    ...
    ev.window_port = FrontWindow();
    em_log.debug_f(
        "Enqueued tagged semantic use torch (what={}, "
        "message=0x{:08X}, when=0x{:08X}, where=(h={}, v={}), "
        "modifiers=0x{:04X})",
        name_for_event_type(ev.what), ev.message, ev.when, ev.where.h,
        ev.where.v, ev.modifiers);
...
Boolean GetNextSemanticGameplayEvent(
  ...
  if ((ret->what == app1Evt) &&
    ...
    uint8_t member = 0;
    uint8_t slot = 0;
    if (still_remastered && RealmzConsumeSemanticUseTorchEvent(
            surface, ret->message, &member, &slot)) {
      // Preserve an application-defined event with no input-shaped payload;
      // the two guarded world loops alone may take the staged source.
      stage_semantic_use_torch_source(member, slot);
      ret->message = 0;
      ret->where = {};
      ret->modifiers = 0;
      ret->window_port = nullptr;
...
Boolean TakeSemanticUseTorchSource(uint8_t* member, uint8_t* slot) {
  const auto pending = pending_semantic_use_torch_source;
  clear_pending_semantic_use_torch_source();
  if (!pending || !member || !slot) {
    return 0;
  }
