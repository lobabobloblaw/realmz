...
void testModeSwitchIsOutsideEngineAndSaveState() {
  ...
  const UIAction showCombatRangeAction{
      .sequence = sequence++,
      .payload = ShowCombatRangeAction{1},
  };
  CHECK(action_name(showCombatRangeAction.payload) == "show_combat_range");
  const auto showCombatRangeUnsupported =
      unsupportedBridge.dispatch(showCombatRangeAction);
  ...
  const UIAction undoCombatantAction{
      .sequence = sequence++,
      .payload = UndoCombatantAction{1},
  };
  CHECK(action_name(undoCombatantAction.payload) == "undo_combatant");
  const auto undoCombatantUnsupported =
      unsupportedBridge.dispatch(undoCombatantAction);
  ...
  const UIAction escapeCombatAction{
      .sequence = sequence++,
      .payload = EscapeCombatAction{1},
  };
  CHECK(action_name(escapeCombatAction.payload) == "escape_combat");
  const auto escapeCombatUnsupported =
      unsupportedBridge.dispatch(escapeCombatAction);
