...
bool consume_set_camp_state(
    RealmzSemanticInputSurface expected_surface,
    uint32_t tag,
    uint32_t& output) {
  return RealmzConsumeSemanticSetCampStateEvent(
             expected_surface, tag, &output) != 0;
}

bool consume_set_search_state(
    RealmzSemanticInputSurface expected_surface,
    uint32_t tag,
    uint8_t& output) {
...
bool consume_contextual_overview(
    RealmzSemanticInputSurface expected_surface,
    uint32_t tag,
    uint32_t& output) {
  return RealmzConsumeSemanticContextualOverviewEvent(
             expected_surface, tag, &output) != 0;
}
...
void test_set_camp_state_tag_encoding_and_collisions() {
  constexpr std::array surfaces{
      REALMZ_SEMANTIC_INPUT_EXPLORATION,
      REALMZ_SEMANTIC_INPUT_DUNGEON,
  };
  std::set<uint32_t> camp_state_tags;
  for (const auto surface : surfaces) {
    for (const bool desired_in_camp : {false, true}) {
      const uint32_t tag =
          semantic_set_camp_state_tag(desired_in_camp, surface);
      CHECK((tag & 0xFFFF0000U) == 0x57430000U);
      CHECK(((tag >> 8U) & 0xFFU) == static_cast<uint32_t>(surface));
      CHECK((tag & 0xFFU) == static_cast<uint32_t>(desired_in_camp));
      CHECK(RealmzIsSemanticSetCampStateTag(tag) != 0);
      CHECK(RealmzSemanticSetCampStateTagSurface(tag) == surface);
      CHECK((RealmzSemanticSetCampStateTagDesiredInCamp(tag) != 0) ==
          desired_in_camp);
      CHECK(RealmzIsSemanticGameplayTag(tag) != 0);
      CHECK(RealmzSemanticGameplayTagSurface(tag) == surface);
      CHECK(RealmzIsSemanticMovementTag(tag) == 0);
      CHECK(RealmzIsSemanticRestPartyTag(tag) == 0);
      CHECK(RealmzIsSemanticCenterActiveCombatantTag(tag) == 0);
      CHECK(camp_state_tags.emplace(tag).second);
    }
  }
  CHECK(camp_state_tags.size() == 4);
  CHECK(semantic_set_camp_state_tag(
      false, REALMZ_SEMANTIC_INPUT_EXPLORATION) == 0x57430100U);
  CHECK(semantic_set_camp_state_tag(
      true, REALMZ_SEMANTIC_INPUT_EXPLORATION) == 0x57430101U);
  CHECK(semantic_set_camp_state_tag(
      false, REALMZ_SEMANTIC_INPUT_DUNGEON) == 0x57430200U);
  CHECK(semantic_set_camp_state_tag(
      true, REALMZ_SEMANTIC_INPUT_DUNGEON) == 0x57430201U);
  CHECK(semantic_set_camp_state_tag(
      true, REALMZ_SEMANTIC_INPUT_NONE) == 0);
  CHECK(semantic_set_camp_state_tag(
      true, REALMZ_SEMANTIC_INPUT_COMBAT) == 0);
  CHECK(semantic_set_camp_state_tag(
      true, static_cast<RealmzSemanticInputSurface>(0xFF)) == 0);
  ...
  for (const uint32_t malformed : {
    ...
    CHECK(RealmzIsSemanticSetCampStateTag(malformed) == 0);
    CHECK(RealmzSemanticSetCampStateTagSurface(malformed) ==
        REALMZ_SEMANTIC_INPUT_NONE);
    CHECK(RealmzSemanticSetCampStateTagDesiredInCamp(malformed) == 0);
  }

  const uint32_t combat_center = semantic_center_active_combatant_tag(
      0, REALMZ_SEMANTIC_INPUT_COMBAT);
  CHECK(combat_center == 0x52430300U);
  CHECK(!camp_state_tags.contains(combat_center));
  CHECK(RealmzIsSemanticSetCampStateTag(combat_center) == 0);
  CHECK(RealmzIsSemanticCenterActiveCombatantTag(combat_center) != 0);
  for (const uint32_t other_tag : {
           semantic_movement_tag(
               MovementCommand::north, REALMZ_SEMANTIC_INPUT_EXPLORATION),
           semantic_party_selection_tag(
    ...
    CHECK(other_tag != 0);
    CHECK(RealmzIsSemanticSetCampStateTag(other_tag) == 0);
  }
}

void test_set_search_state_tag_encoding_and_collisions() {
  constexpr std::array surfaces{
      REALMZ_SEMANTIC_INPUT_EXPLORATION,
      REALMZ_SEMANTIC_INPUT_DUNGEON,
  };
  std::set<uint32_t> search_state_tags;
  for (const auto surface : surfaces) {
    for (const bool desired_searching : {false, true}) {
      const uint32_t tag =
          semantic_set_search_state_tag(desired_searching, surface);
      CHECK((tag & 0xFFFF0000U) == 0x57530000U);
      CHECK(((tag >> 8U) & 0xFFU) == static_cast<uint32_t>(surface));
      CHECK((tag & 0xFFU) == static_cast<uint32_t>(desired_searching));
      CHECK(RealmzIsSemanticSetSearchStateTag(tag) != 0);
      CHECK(RealmzSemanticSetSearchStateTagSurface(tag) == surface);
      CHECK((RealmzSemanticSetSearchStateTagDesiredSearching(tag) != 0) ==
          desired_searching);
      CHECK(RealmzIsSemanticGameplayTag(tag) != 0);
      CHECK(RealmzSemanticGameplayTagSurface(tag) == surface);
      CHECK(RealmzIsSemanticMovementTag(tag) == 0);
      ...
      CHECK(search_state_tags.emplace(tag).second);
    }
  }
  CHECK(search_state_tags.size() == 4);
  CHECK(semantic_set_search_state_tag(
      false, REALMZ_SEMANTIC_INPUT_EXPLORATION) == 0x57530100U);
  CHECK(semantic_set_search_state_tag(
      true, REALMZ_SEMANTIC_INPUT_EXPLORATION) == 0x57530101U);
  CHECK(semantic_set_search_state_tag(
      false, REALMZ_SEMANTIC_INPUT_DUNGEON) == 0x57530200U);
  CHECK(semantic_set_search_state_tag(
      true, REALMZ_SEMANTIC_INPUT_DUNGEON) == 0x57530201U);
  CHECK(semantic_set_search_state_tag(
      true, REALMZ_SEMANTIC_INPUT_NONE) == 0);
  CHECK(semantic_set_search_state_tag(
      true, REALMZ_SEMANTIC_INPUT_COMBAT) == 0);
  ...
  for (const uint32_t malformed : {
    ...
    CHECK(RealmzIsSemanticSetSearchStateTag(malformed) == 0);
    CHECK(RealmzSemanticSetSearchStateTagSurface(malformed) ==
        REALMZ_SEMANTIC_INPUT_NONE);
    CHECK(RealmzSemanticSetSearchStateTagDesiredSearching(malformed) == 0);
  }
...
void test_contextual_overview_tag_encoding_and_collisions() {
  constexpr std::array surfaces{
      REALMZ_SEMANTIC_INPUT_EXPLORATION,
      REALMZ_SEMANTIC_INPUT_DUNGEON,
  };
  ...
  std::set<uint32_t> overview_tags;
  for (const auto surface : surfaces) {
    const uint32_t area_tag = semantic_contextual_overview_tag(area, surface);
    CHECK(area_tag == (0x574F0000U |
        (static_cast<uint32_t>(surface) << 8U)));
    CHECK(RealmzIsSemanticContextualOverviewTag(area_tag) != 0);
    CHECK(RealmzSemanticContextualOverviewTagSurface(area_tag) == surface);
    CHECK(RealmzSemanticContextualOverviewTagIsAreaSearch(area_tag) != 0);
    CHECK(RealmzIsSemanticGameplayTag(area_tag) != 0);
    CHECK(RealmzSemanticGameplayTagSurface(area_tag) == surface);
    CHECK(RealmzIsSemanticRestPartyTag(area_tag) == 0);
    CHECK(RealmzIsSemanticSetCampStateTag(area_tag) == 0);
    CHECK(RealmzIsSemanticSetSearchStateTag(area_tag) == 0);
    CHECK(RealmzIsSemanticUseTorchTag(area_tag) == 0);
    CHECK(overview_tags.emplace(area_tag).second);

    for (PartyMemberId member = 0; member <= 5; ++member) {
      const ContextualOverviewAction make{
          .mode = ContextualOverviewMode::make_scroll,
          .member = member,
      };
      const uint32_t make_tag =
          semantic_contextual_overview_tag(make, surface);
      CHECK(make_tag == (0x574F0000U |
          (static_cast<uint32_t>(surface) << 8U) | 0x80U | member));
      CHECK(RealmzIsSemanticContextualOverviewTag(make_tag) != 0);
      CHECK(RealmzSemanticContextualOverviewTagSurface(make_tag) == surface);
      CHECK(RealmzSemanticContextualOverviewTagIsAreaSearch(make_tag) == 0);
      CHECK(RealmzIsSemanticGameplayTag(make_tag) != 0);
      CHECK(RealmzSemanticGameplayTagSurface(make_tag) == surface);
      CHECK(overview_tags.emplace(make_tag).second);
    }
  }
  CHECK(overview_tags.size() == 14);
  CHECK(semantic_contextual_overview_tag(
      area, REALMZ_SEMANTIC_INPUT_EXPLORATION) == 0x574F0100U);
  CHECK(semantic_contextual_overview_tag(
      ContextualOverviewAction{
          .mode = ContextualOverviewMode::make_scroll,
          .member = 5,
      },
      REALMZ_SEMANTIC_INPUT_DUNGEON) == 0x574F0285U);

  for (const RealmzSemanticInputSurface surface :
       std::array<RealmzSemanticInputSurface, 3>{
           REALMZ_SEMANTIC_INPUT_NONE,
           REALMZ_SEMANTIC_INPUT_COMBAT,
           0xFF,
       }) {
    CHECK(semantic_contextual_overview_tag(area, surface) == 0);
  }
  for (const auto& invalid : {
    ...
    CHECK(semantic_contextual_overview_tag(
        invalid, REALMZ_SEMANTIC_INPUT_EXPLORATION) == 0);
  }
  ...
  for (const uint32_t malformed : {
    ...
    CHECK(RealmzIsSemanticContextualOverviewTag(malformed) == 0);
    CHECK(RealmzSemanticContextualOverviewTagSurface(malformed) ==
        REALMZ_SEMANTIC_INPUT_NONE);
    CHECK(RealmzSemanticContextualOverviewTagIsAreaSearch(malformed) == 0);
    CHECK(RealmzIsSemanticGameplayTag(malformed) == 0);
    CHECK(RealmzSemanticGameplayTagSurface(malformed) ==
        REALMZ_SEMANTIC_INPUT_NONE);
  }
...
void test_set_camp_state_late_validation_and_exact_translation() {
  reset_capture(
      REALMZ_LEGACY_SCREEN_EXPLORATION,
      ScreenContext::exploration,
      WorldPresentation::outdoor);
  captured_snapshot.world.in_camp = false;
  const uint32_t enter_camp = semantic_set_camp_state_tag(
      true, REALMZ_SEMANTIC_INPUT_EXPLORATION);
  const uint32_t leave_camp = semantic_set_camp_state_tag(
      false, REALMZ_SEMANTIC_INPUT_EXPLORATION);
  const uint32_t combat_center = semantic_center_active_combatant_tag(
      0, REALMZ_SEMANTIC_INPUT_COMBAT);
  uint32_t output = kUnchangedClassicMessage;

  CHECK(!consume_set_camp_state(
      REALMZ_SEMANTIC_INPUT_EXPLORATION, enter_camp, output));
  CHECK(output == kUnchangedClassicMessage);
  CHECK(legacy_capture_calls == 0);
  CHECK(snapshot_capture_calls == 0);
  ...
  CHECK(consume_set_camp_state(
      REALMZ_SEMANTIC_INPUT_EXPLORATION, enter_camp, output));
  CHECK(output == 0x00000863U);
  CHECK(legacy_capture_calls == 1);
  CHECK(snapshot_capture_calls == 1);

  // Authorization is one-shot, and a missing output burns it before any
  // mutable Classic state or fresh snapshot is consulted.
  output = kUnchangedClassicMessage;
  CHECK(!consume_set_camp_state(
      REALMZ_SEMANTIC_INPUT_EXPLORATION, enter_camp, output));
  CHECK(output == kUnchangedClassicMessage);
  complete_top_level_scope(REALMZ_SEMANTIC_INPUT_EXPLORATION);
  CHECK(RealmzConsumeSemanticSetCampStateEvent(
            REALMZ_SEMANTIC_INPUT_EXPLORATION,
            enter_camp,
            nullptr) == 0);
  CHECK(legacy_capture_calls == 1);
  ...
  // Combat Center shares lowercase-c only after translation; its typed tag
  // cannot enter the world Camp consumer.
  complete_top_level_scope(REALMZ_SEMANTIC_INPUT_EXPLORATION);
  CHECK(!consume_set_camp_state(
      REALMZ_SEMANTIC_INPUT_EXPLORATION, combat_center, output));
  CHECK(output == kUnchangedClassicMessage);
  CHECK(legacy_capture_calls == 1);
  CHECK(snapshot_capture_calls == 1);

  // Both desired directions translate exactly once while the fresh current
  // state differs, and become inert once it already matches.
  reset_capture(
      REALMZ_LEGACY_SCREEN_EXPLORATION,
      ScreenContext::exploration,
      WorldPresentation::outdoor);
  ...
  output = kUnchangedClassicMessage;
  CHECK(consume_set_camp_state(
      REALMZ_SEMANTIC_INPUT_EXPLORATION, leave_camp, output));
  CHECK(output == 0x00000863U);
  ...
  for (const auto presentation : {
    ...
    reset_capture(
        ...
        presentation);
    captured_snapshot.world.in_camp = false;
    const uint32_t dungeon_enter = semantic_set_camp_state_tag(
        true, REALMZ_SEMANTIC_INPUT_DUNGEON);
    complete_top_level_scope(REALMZ_SEMANTIC_INPUT_DUNGEON);
    output = kUnchangedClassicMessage;
    CHECK(consume_set_camp_state(
        REALMZ_SEMANTIC_INPUT_DUNGEON, dungeon_enter, output));
    CHECK(output == 0x00000863U);
  }

  // Originating-surface mismatch fails before context capture.
  reset_capture(
      REALMZ_LEGACY_SCREEN_DUNGEON,
      ScreenContext::dungeon,
      WorldPresentation::dungeon_map);
  ...
  output = kUnchangedClassicMessage;
  CHECK(!consume_set_camp_state(
      REALMZ_SEMANTIC_INPUT_DUNGEON, enter_camp, output));
  CHECK(output == kUnchangedClassicMessage);
  CHECK(legacy_capture_calls == 0);
  CHECK(snapshot_capture_calls == 0);
  ...
  CHECK(!consume_set_camp_state(
      REALMZ_SEMANTIC_INPUT_DUNGEON,
      semantic_set_camp_state_tag(true, REALMZ_SEMANTIC_INPUT_DUNGEON),
      output));
  CHECK(output == kUnchangedClassicMessage);
...
void test_set_search_state_late_validation_and_absolute_handoff() {
  reset_capture(
      REALMZ_LEGACY_SCREEN_EXPLORATION,
      ScreenContext::exploration,
      WorldPresentation::outdoor);
  captured_snapshot.world.searching = false;
  const uint32_t start_searching = semantic_set_search_state_tag(
      true, REALMZ_SEMANTIC_INPUT_EXPLORATION);
  const uint32_t stop_searching = semantic_set_search_state_tag(
      false, REALMZ_SEMANTIC_INPUT_EXPLORATION);
  uint8_t output = 0xA5;

  CHECK(!consume_set_search_state(
      REALMZ_SEMANTIC_INPUT_EXPLORATION, start_searching, output));
  CHECK(output == 0xA5);
  CHECK(legacy_capture_calls == 0);
  CHECK(snapshot_capture_calls == 0);
  ...
  // Authorization and the desired state are each one-shot. A null output also
  // burns a newly completed authorization before consulting live state.
  output = 0xA5;
  CHECK(!consume_set_search_state(
      REALMZ_SEMANTIC_INPUT_EXPLORATION, start_searching, output));
  CHECK(output == 0xA5);
  complete_top_level_scope(REALMZ_SEMANTIC_INPUT_EXPLORATION);
  CHECK(RealmzConsumeSemanticSetSearchStateEvent(
            REALMZ_SEMANTIC_INPUT_EXPLORATION,
            start_searching,
            nullptr) == 0);
  CHECK(legacy_capture_calls == 1);
  ...
  // A same-shaped Camp tag cannot enter Search's no-key handoff.
  complete_top_level_scope(REALMZ_SEMANTIC_INPUT_EXPLORATION);
  CHECK(!consume_set_search_state(
      REALMZ_SEMANTIC_INPUT_EXPLORATION,
      semantic_set_camp_state_tag(
  ...
  complete_top_level_scope(REALMZ_SEMANTIC_INPUT_EXPLORATION);
  CHECK(consume_set_search_state(
      REALMZ_SEMANTIC_INPUT_EXPLORATION, stop_searching, output));
  CHECK(output == 0);
  ...
  // A tag from the wrong originating surface fails before any context capture.
  reset_capture(
      REALMZ_LEGACY_SCREEN_DUNGEON,
      ScreenContext::dungeon,
      WorldPresentation::dungeon_map);
  ...
  output = 0xA5;
  CHECK(!consume_set_search_state(
      REALMZ_SEMANTIC_INPUT_DUNGEON, start_searching, output));
  CHECK(output == 0xA5);
  CHECK(legacy_capture_calls == 0);
  CHECK(snapshot_capture_calls == 0);
  ...
  CHECK(!consume_set_search_state(
      REALMZ_SEMANTIC_INPUT_DUNGEON,
      semantic_set_search_state_tag(true, REALMZ_SEMANTIC_INPUT_DUNGEON),
      output));
  CHECK(output == 0xA5);
...
void test_contextual_overview_late_validation_and_exact_translation() {
  const ContextualOverviewAction area{
      .mode = ContextualOverviewMode::area_search,
  };
  const ContextualOverviewAction make{
  ...
  const uint32_t area_tag = semantic_contextual_overview_tag(
      area, REALMZ_SEMANTIC_INPUT_EXPLORATION);
  const uint32_t make_tag = semantic_contextual_overview_tag(
      make, REALMZ_SEMANTIC_INPUT_EXPLORATION);
  uint32_t output = kUnchangedClassicMessage;
  ...
  CHECK(!consume_contextual_overview(
      REALMZ_SEMANTIC_INPUT_EXPLORATION, area_tag, output));
  CHECK(output == kUnchangedClassicMessage);
  CHECK(legacy_capture_calls == 0);
  CHECK(snapshot_capture_calls == 0);

  // Area Search remains party-wide and lets Classic own fatigue feedback.
  complete_top_level_scope(REALMZ_SEMANTIC_INPUT_EXPLORATION);
  CHECK(consume_contextual_overview(
      REALMZ_SEMANTIC_INPUT_EXPLORATION, area_tag, output));
  CHECK(output == 0x00000061U);
  CHECK(legacy_capture_calls == 1);
  CHECK(snapshot_capture_calls == 1);
  ...
  CHECK(output == kUnchangedClassicMessage);
  complete_top_level_scope(REALMZ_SEMANTIC_INPUT_EXPLORATION);
  CHECK(RealmzConsumeSemanticContextualOverviewEvent(
            REALMZ_SEMANTIC_INPUT_EXPLORATION,
            area_tag,
            nullptr) == 0);
  CHECK(legacy_capture_calls == 1);
  ...
  // Fresh camp state rejects Area Search and consumes the authorization.
  captured_snapshot.world.in_camp = true;
  complete_top_level_scope(REALMZ_SEMANTIC_INPUT_EXPLORATION);
  CHECK(!consume_contextual_overview(
      REALMZ_SEMANTIC_INPUT_EXPLORATION, area_tag, output));
  ...
  for (const auto presentation : {
    ...
    reset_capture(
        ...
        presentation);
    captured_snapshot.world.in_camp = false;
    const uint32_t dungeon_area = semantic_contextual_overview_tag(
        area, REALMZ_SEMANTIC_INPUT_DUNGEON);
    complete_top_level_scope(REALMZ_SEMANTIC_INPUT_DUNGEON);
    output = kUnchangedClassicMessage;
    CHECK(consume_contextual_overview(
        REALMZ_SEMANTIC_INPUT_DUNGEON, dungeon_area, output));
    CHECK(output == 0x00000061U);
  }

  // The tag's originating surface is checked before either live capture.
  reset_capture(
      REALMZ_LEGACY_SCREEN_DUNGEON,
      ScreenContext::dungeon,
      WorldPresentation::dungeon_map);
  ...
  output = kUnchangedClassicMessage;
  CHECK(!consume_contextual_overview(
      REALMZ_SEMANTIC_INPUT_DUNGEON, area_tag, output));
  CHECK(output == kUnchangedClassicMessage);
  CHECK(legacy_capture_calls == 0);
  CHECK(snapshot_capture_calls == 0);
  ...
  // Make Scroll binds and freshly validates one exact selected member. Caste
  // is deliberately irrelevant here so Classic can retain its noncaster warn.
  reset_capture(
      REALMZ_LEGACY_SCREEN_EXPLORATION,
      ScreenContext::exploration,
      WorldPresentation::outdoor);
  captured_snapshot.world.in_camp = true;
  captured_snapshot.party.members[0].caste_id = 99;
  complete_top_level_scope(REALMZ_SEMANTIC_INPUT_EXPLORATION);
  output = kUnchangedClassicMessage;
  CHECK(consume_contextual_overview(
      REALMZ_SEMANTIC_INPUT_EXPLORATION, make_tag, output));
  CHECK(output == 0x0000286BU);

  const GameSnapshot valid_make_snapshot = captured_snapshot;
  captured_snapshot.world.in_camp = false;
  complete_top_level_scope(REALMZ_SEMANTIC_INPUT_EXPLORATION);
  output = kUnchangedClassicMessage;
  CHECK(!consume_contextual_overview(
  ...
  captured_snapshot = valid_make_snapshot;
  captured_snapshot.party.selected_member = 1;
  captured_snapshot.party.members[0].selected = false;
  captured_snapshot.party.members[1].selected = true;
  complete_top_level_scope(REALMZ_SEMANTIC_INPUT_EXPLORATION);
  ...
  captured_snapshot = valid_make_snapshot;
  const uint32_t missing_member_tag = semantic_contextual_overview_tag(
      ContextualOverviewAction{
          .mode = ContextualOverviewMode::make_scroll,
          .member = 5,
      },
  ...
  complete_top_level_scope(REALMZ_SEMANTIC_INPUT_EXPLORATION);
  CHECK(!consume_contextual_overview(
      REALMZ_SEMANTIC_INPUT_EXPLORATION, missing_member_tag, output));
  ...
  for (const auto presentation : {
    ...
    reset_capture(
        ...
        presentation);
    captured_snapshot.world.in_camp = true;
    const uint32_t dungeon_make = semantic_contextual_overview_tag(
        make, REALMZ_SEMANTIC_INPUT_DUNGEON);
    complete_top_level_scope(REALMZ_SEMANTIC_INPUT_DUNGEON);
    output = kUnchangedClassicMessage;
    CHECK(consume_contextual_overview(
        REALMZ_SEMANTIC_INPUT_DUNGEON, dungeon_make, output));
    CHECK(output == 0x0000286BU);
  }
