...
constexpr uint32_t kSemanticSetCampStateSignature = 0x57430000U;
constexpr uint32_t kSemanticSetCampStateMask = 0xFFFF0000U;
constexpr uint32_t kSemanticSetCampStateSurfaceMask = 0x0000FF00U;
constexpr uint32_t kSemanticSetCampStateDesiredMask = 0x000000FFU;
constexpr uint32_t kSemanticSetSearchStateSignature = 0x57530000U;
...
constexpr uint32_t kSemanticContextualOverviewSignature = 0x574F0000U;
constexpr uint32_t kSemanticContextualOverviewMask = 0xFFFF0000U;
constexpr uint32_t kSemanticContextualOverviewSurfaceMask = 0x0000FF00U;
constexpr uint32_t kSemanticContextualOverviewPayloadMask = 0x000000FFU;
...
std::optional<DecodedSetCampState> decode_set_camp_state(
    uint32_t tagged_message) noexcept {
  if ((tagged_message & kSemanticSetCampStateMask) !=
      kSemanticSetCampStateSignature) {
    return std::nullopt;
  }
  const uint32_t surface_value =
      (tagged_message & kSemanticSetCampStateSurfaceMask) >> 8U;
  const uint32_t desired_value =
      tagged_message & kSemanticSetCampStateDesiredMask;
  if (!is_world_gameplay_surface(surface_value) || desired_value > 1U) {
    return std::nullopt;
  }
  return DecodedSetCampState{
      .desired_in_camp = desired_value != 0,
      .surface = surface_value,
  };
}

std::optional<DecodedSetSearchState> decode_set_search_state(
    uint32_t tagged_message) noexcept {
  if ((tagged_message & kSemanticSetSearchStateMask) !=
      kSemanticSetSearchStateSignature) {
    return std::nullopt;
  }
  const uint32_t surface_value =
      (tagged_message & kSemanticSetSearchStateSurfaceMask) >> 8U;
  const uint32_t desired_value =
      tagged_message & kSemanticSetSearchStateDesiredMask;
  if (!is_world_gameplay_surface(surface_value) || desired_value > 1U) {
    return std::nullopt;
  }
...
std::optional<DecodedContextualOverview> decode_contextual_overview(
    uint32_t tagged_message) noexcept {
  if ((tagged_message & kSemanticContextualOverviewMask) !=
      kSemanticContextualOverviewSignature) {
    return std::nullopt;
  }
  const uint32_t surface_value =
      (tagged_message & kSemanticContextualOverviewSurfaceMask) >> 8U;
  const auto payload = static_cast<uint8_t>(
      tagged_message & kSemanticContextualOverviewPayloadMask);
  if (!is_world_gameplay_surface(surface_value)) {
    return std::nullopt;
  }
  if (payload == 0) {
    return DecodedContextualOverview{
        .action = {
            .mode = realmz::presentation::ContextualOverviewMode::area_search,
            .member = std::nullopt,
        },
        .surface = surface_value,
    };
  }
  if ((payload < kSemanticContextualOverviewMakeScrollFlag) ||
  ...
  return DecodedContextualOverview{
      .action = {
          .mode = realmz::presentation::ContextualOverviewMode::make_scroll,
          .member = static_cast<realmz::presentation::PartyMemberId>(
              payload - kSemanticContextualOverviewMakeScrollFlag),
      },
      .surface = surface_value,
...
uint32_t semantic_set_camp_state_tag(
    bool desired_in_camp,
    RealmzSemanticInputSurface surface) noexcept {
  if (!is_world_gameplay_surface(surface)) {
    return 0;
  }
  return kSemanticSetCampStateSignature |
      (static_cast<uint32_t>(surface) << 8U) |
      static_cast<uint32_t>(desired_in_camp);
}

uint32_t semantic_set_search_state_tag(
    bool desired_searching,
    RealmzSemanticInputSurface surface) noexcept {
  if (!is_world_gameplay_surface(surface)) {
    return 0;
  }
  return kSemanticSetSearchStateSignature |
      (static_cast<uint32_t>(surface) << 8U) |
...
uint32_t semantic_contextual_overview_tag(
  ...
  switch (action.mode) {
    ...
    case ContextualOverviewMode::make_scroll:
      if (!action.member ||
          (*action.member > kMaximumContextualOverviewMember)) {
        return 0;
      }
      payload = kSemanticContextualOverviewMakeScrollFlag + *action.member;
      break;
    default:
      return 0;
  }
  return kSemanticContextualOverviewSignature |
...
extern "C" uint8_t RealmzIsSemanticSetCampStateTag(
    uint32_t tagged_message) {
  return decode_set_camp_state(tagged_message).has_value() ? 1 : 0;
}

extern "C" RealmzSemanticInputSurface
RealmzSemanticSetCampStateTagSurface(uint32_t tagged_message) {
  const auto camp_state = decode_set_camp_state(tagged_message);
  return camp_state ? camp_state->surface : kNoSemanticInputSurface;
}

extern "C" uint8_t RealmzSemanticSetCampStateTagDesiredInCamp(
    uint32_t tagged_message) {
  const auto camp_state = decode_set_camp_state(tagged_message);
  return camp_state && camp_state->desired_in_camp ? 1 : 0;
}

extern "C" uint8_t RealmzIsSemanticSetSearchStateTag(
    uint32_t tagged_message) {
  return decode_set_search_state(tagged_message).has_value() ? 1 : 0;
}

extern "C" RealmzSemanticInputSurface
RealmzSemanticSetSearchStateTagSurface(uint32_t tagged_message) {
  const auto search_state = decode_set_search_state(tagged_message);
  return search_state ? search_state->surface : kNoSemanticInputSurface;
}

extern "C" uint8_t RealmzSemanticSetSearchStateTagDesiredSearching(
    uint32_t tagged_message) {
  const auto search_state = decode_set_search_state(tagged_message);
...
extern "C" uint8_t RealmzIsSemanticContextualOverviewTag(
    uint32_t tagged_message) {
  return decode_contextual_overview(tagged_message).has_value() ? 1 : 0;
}

extern "C" RealmzSemanticInputSurface
RealmzSemanticContextualOverviewTagSurface(uint32_t tagged_message) {
  const auto overview = decode_contextual_overview(tagged_message);
  return overview ? overview->surface : kNoSemanticInputSurface;
}

extern "C" uint8_t RealmzSemanticContextualOverviewTagIsAreaSearch(
    uint32_t tagged_message) {
  const auto overview = decode_contextual_overview(tagged_message);
  return overview &&
      (overview->action.mode ==
          realmz::presentation::ContextualOverviewMode::area_search)
      ? 1
      : 0;
}
...
RealmzSemanticGameplayTagSurface(uint32_t tagged_message) {
  ...
  if (const auto camp_state = decode_set_camp_state(tagged_message)) {
    return camp_state->surface;
  }
...
extern "C" uint8_t RealmzConsumeSemanticSetCampStateEvent(
    RealmzSemanticInputSurface expected_surface,
    uint32_t tagged_message,
    uint32_t* classic_key_message) {
  const bool authorized = authorize_completed_scope(expected_surface);
  ...
  const auto camp_state = decode_set_camp_state(tagged_message);
  if (!camp_state || (camp_state->surface != expected_surface)) {
    return 0;
  }
  ...
  try {
    ...
    const auto message =
        realmz::presentation::legacy_key_message_for_set_camp_state(
...
extern "C" uint8_t RealmzConsumeSemanticSetSearchStateEvent(
    RealmzSemanticInputSurface expected_surface,
    uint32_t tagged_message,
    uint8_t* desired_searching) {
  const bool authorized = authorize_completed_scope(expected_surface);
  if (!desired_searching || !authorized) {
    return 0;
  }
  const auto search_state = decode_set_search_state(tagged_message);
  if (!search_state || (search_state->surface != expected_surface)) {
    return 0;
  }
    ...
    if ((snapshot.screen != screen) ||
        !realmz::presentation::
            runtime_legacy_context_supports_set_search_state(
                search_state->desired_searching, context)) {
      return 0;
    }
...
extern "C" uint8_t RealmzConsumeSemanticContextualOverviewEvent(
    RealmzSemanticInputSurface expected_surface,
    uint32_t tagged_message,
    uint32_t* classic_key_message) {
  const bool authorized = authorize_completed_scope(expected_surface);
  ...
  const auto overview = decode_contextual_overview(tagged_message);
  if (!overview || (overview->surface != expected_surface)) {
    return 0;
  }
  ...
  try {
    ...
    if (overview->action.mode ==
        realmz::presentation::ContextualOverviewMode::make_scroll) {
      if (!overview->action.member) {
        return 0;
      }
      const auto* member = snapshot.party.member(*overview->action.member);
      if (!member || !member->selected || !member->use_scroll_available ||
