...
constexpr auto kGameplayChromeCoverageManifest = std::to_array<Entry>({
    ...
    {"exploration.action.selected_item_drilldown", Surface::exploration,
        Kind::interaction, Status::semantic_complete,
        "The PARTY deck exposes EQUIPMENT as a typed selected-member Classic quick equip/unequip popup, distinct from full ITEMS. Its strict single-use 0x5349SSMM tag binds outdoor surface byte 0x01 and member byte 0x00..0x05. Late validation requires the completed semantic scope, adaptive outdoor presentation, and the same freshly selected member before EventManager yields one neutral app1Evt with zero modifiers and stages that member. It forges no key or pointer and has no held-mouse gate. The outdoor loop alone selects the real showitembut; Classic buttonchoice and showcondition own popup browsing plus wear/removeitem mutation. No replay vocabulary is added.",
        "src/presentation/SemanticInputBoundary.cpp::semantic_selected_item_drilldown_tag/RealmzConsumeSemanticSelectedItemDrilldownEvent"},
    {"exploration.action.contextual_shop_temple_encounter",
        Surface::exploration, Kind::interaction, Status::semantic_complete,
        "The GAME deck uses one typed ContextualWorldEntryAction in Rest's mutually exclusive outside-camp slot, labeled SHOP, TEMPLE, or ENCOUNTER from the strict shopavail > templeavail > encounter priority while ignoring visual-only canshop. Its single-use 0x5745SSMM tag binds outdoor surface 0x01 and explicit mode 0/1/2; late adaptive outdoor, non-camp, and exact-mode validation yields only neutral Classic g 0x00000567 or e 0x00000E65 keyDown delivery with no held-mouse gate. Classic buttonchoice retains macro, shop/temple modal, RNG, save, and seamless-transition ownership; no Classic or replay vocabulary changes.",
        "src/presentation/SemanticInputBoundary.cpp::semantic_contextual_world_entry_tag/RealmzConsumeSemanticContextualWorldEntryEvent"},
    {"exploration.action.pool_money", Surface::exploration,
        Kind::interaction, Status::semantic_complete,
        "The PARTY page has one member-free MONEY control in its sixth slot. Its empty OpenMoneyManagementAction uses a strict single-use 0x574DSS00 tag with outdoor SS=0x01 and reserved low byte zero. Completed-scope delivery freshly requires adaptive outdoor presentation, a nonempty party of at most six, and a valid current selected member without carrying or binding member identity, then yields only Classic's exact lowercase m keyDown 0x00002E6D through a neutral one-shot app1Evt with no held-mouse gate. Camp and noncamp are both valid; dead swapavail and funds/shop/temple/bank state do not gate opening. Classic swapbut/CNTL 157, checkkeypad, buttonchoice, swap, pool, and share retain modal and effect ownership. No Classic source or replay vocabulary changes; private no-redistribution manual QA remains open. This interaction does not cover pooled-money information.",
        "src/presentation/SemanticInputBoundary.cpp::semantic_open_money_management_tag/RealmzConsumeSemanticOpenMoneyManagementEvent"},
    ...
    {"exploration.info.party_vitals", Surface::exploration,
        ...
        "The shared code-native party rail presents every member during outdoor exploration with row 1 name, Lv, and AC; row 2 stamina plus SP for every caster selected solely by nonzero Classic spellpointsmax, including zero current SP, or ATK cadence for a noncaster; and row 3 state summary. Detached capture copies ac, spellpoints/max, normattacks, attackbonus, and conditions. Cadence follows Classic updatechar exactly: raw normattacks + attackbonus, Speedy condition 23 doubles first, then Slow condition 6 integer-halves. Adjusted values 0..19 render as reduced half fractions and every other value renders `> 10`. Each layout retains complete unelided semantic accessibility text for every row and state; the outdoor selection control carries it in accessibility-label metadata, but this slice makes no OS-publication claim. The rail remains read-only and introduces no action, tag, input, Classic-source, or replay-vocabulary change.",
        "src/presentation/PartyRailLayout.cpp::compute_party_rail_layout/layout_member"},
    ...
    {"exploration.info.party_condition_indicators", Surface::exploration,
        Kind::essential_information, Status::semantic_complete,
        "The shared code-native PARTY STATUS ribbon presents Classic's ordered party-wide effect indicators during outdoor exploration. Detached capture copies exactly signed partycondition indices 1 through 8, excluding Torch index 0 and unused index 9; every nonzero value, including negative Search or equipment sentinels, is active. The model preserves Classic index order and labels Waterworld, Dragon Hide, Discover Secret, Wizard Eye, Search, Free Fall / Levitate, Sentry, and Charm Resistance without describing raw values as durations. Layout retains every complete token and unelided semantic accessibility string while using deterministic visible +N elision only when needed. This read-only information path makes no OS-publication claim and introduces no action, tag, input, Classic-source, or replay-vocabulary change.",
        "src/presentation/PartyRailLayout.cpp::compute_party_rail_layout"},
    ...
    {"exploration.info.world_coordinates", Surface::exploration,
        Kind::essential_information, Status::semantic_complete,
        "The shared responsive action-header world-context strip beside the persistent action-page tabs presents world-only outdoor coordinates during exploration. It derives exact signed display values without clamping from checked int32 lookx + partyx and looky + partyy additions. Any nonzero xydisplayflag instead conceals both coordinates as literal ? values with no raw-coordinate leakage. The strip is absent in combat and retains complete internal semantic text without an OS-publication claim. This read-only information path adds no action, tag, input, Classic-source, or replay-vocabulary change.",
        "src/presentation/WorldContextLayout.cpp::compute_world_context_layout"},
    {"exploration.info.calendar_clock", Surface::exploration,
        Kind::essential_information, Status::semantic_complete,
        "The shared responsive action-header world-context strip beside the persistent action-page tabs presents world-only campaign day and time during outdoor exploration. Campaign day is displayed at Classic's signed-short boundary with no +1 and no calendar conversion. Time is a deterministic %I:%M %p-equivalent, zero-padded English 12-hour rendering derived from raw hour and minute with no locale or timezone normalization. The strip is absent in combat and retains complete internal semantic text without an OS-publication claim. This read-only information path adds no action, tag, input, Classic-source, or replay-vocabulary change.",
        "src/presentation/WorldContextLayout.cpp::compute_world_context_layout"},
    {"exploration.info.fatigue", Surface::exploration,
        Kind::essential_information, Status::semantic_complete,
        "The outdoor PARTY STATUS ribbon presents Classic's exact signed fatigue as FAT raw/135 with a bounded meter and explicit non-color band: baseline through 70, elevated from 71 through 105, and critical above 105, matching updatefat's two strict thresholds without normalizing the retained raw value. Fatigue remains absent from the combat ribbon because Classic returns before drawing it in combat. Complete internal semantic accessibility text is retained without claiming OS publication; no action, tag, input, Classic-source, or replay-vocabulary path is added.",
        "src/presentation/PartyRailLayout.cpp::compute_party_rail_layout"},
    {"exploration.info.pooled_money", Surface::exploration,
        Kind::essential_information, Status::semantic_complete,
        "The outdoor PARTY STATUS ribbon presents exact signed moneypool indices 0, 1, and 2 in Classic Gold, Gems, Jewelry order with no clamp, denomination conversion, or substitution of member or bank holdings. Pooled money remains absent from combat, where the canonical inventory defines no pooled-money role. Complete internal semantic accessibility text is retained without claiming OS publication; no action, tag, input, Classic-source, or replay-vocabulary path is added.",
        "src/presentation/PartyRailLayout.cpp::compute_party_rail_layout"},
    {"exploration.info.search_and_torch_state", Surface::exploration,
        Kind::essential_information, Status::semantic_complete,
        "The shared responsive action-header world-context strip beside the persistent action-page tabs presents world-only Search from raw signed partycondition[5] and Torch from raw signed partycondition[0] during outdoor exploration. Any nonzero value, including negative, is active; exact signed raw values are retained and displayed without calling either value a duration, turns, or charges. The usable Torch source remains action-availability data only. Both active and inactive states use explicit non-color markers. The strip is absent in combat and retains complete internal semantic text without an OS-publication claim. This read-only information path adds no action, tag, input, Classic-source, or replay-vocabulary change.",
        "src/presentation/WorldContextLayout.cpp::compute_world_context_layout"},
    ...
    {"dungeon.action.selected_item_drilldown", Surface::dungeon,
        Kind::interaction, Status::semantic_complete,
        "The PARTY deck exposes EQUIPMENT as a typed selected-member Classic quick equip/unequip popup, distinct from full ITEMS. Its strict single-use 0x5349SSMM tag binds dungeon surface byte 0x02 and member byte 0x00..0x05. Late validation requires the completed semantic scope, adaptive dungeon-map or first-person presentation, and the same freshly selected member before EventManager yields one neutral app1Evt with zero modifiers and stages that member. It forges no key or pointer and has no held-mouse gate. The dungeon loop alone selects the real showitembut; Classic buttonchoice and showcondition own popup browsing plus wear/removeitem mutation. No replay vocabulary is added.",
        "src/presentation/SemanticInputBoundary.cpp::semantic_selected_item_drilldown_tag/RealmzConsumeSemanticSelectedItemDrilldownEvent"},
    {"dungeon.action.contextual_shop_temple_encounter", Surface::dungeon,
        Kind::interaction, Status::semantic_complete,
        "The GAME deck uses one typed ContextualWorldEntryAction in Rest's mutually exclusive outside-camp slot, labeled SHOP, TEMPLE, or ENCOUNTER from the strict shopavail > templeavail > encounter priority while ignoring visual-only canshop. Its single-use 0x5745SSMM tag binds dungeon surface 0x02 and explicit mode 0/1/2; late adaptive dungeon-map or first-person, non-camp, and exact-mode validation yields only neutral Classic g 0x00000567 or e 0x00000E65 keyDown delivery with no held-mouse gate. Classic buttonchoice retains macro, shop/temple modal, RNG, save, and seamless-transition ownership; no Classic or replay vocabulary changes.",
        "src/presentation/SemanticInputBoundary.cpp::semantic_contextual_world_entry_tag/RealmzConsumeSemanticContextualWorldEntryEvent"},
    {"dungeon.action.pool_money", Surface::dungeon,
        Kind::interaction, Status::semantic_complete,
        "The PARTY page has one member-free MONEY control in its sixth slot. Its empty OpenMoneyManagementAction uses a strict single-use 0x574DSS00 tag with dungeon SS=0x02 and reserved low byte zero. Completed-scope delivery freshly requires adaptive dungeon-map or first-person presentation, a nonempty party of at most six, and a valid current selected member without carrying or binding member identity, then yields only Classic's exact lowercase m keyDown 0x00002E6D through a neutral one-shot app1Evt with no held-mouse gate. Camp and noncamp are both valid; dead swapavail and funds/shop/temple/bank state do not gate opening. Classic swapbut/CNTL 157, threed, buttonchoice, swap, pool, and share retain modal and effect ownership. No Classic source or replay vocabulary changes; private no-redistribution manual QA remains open. This interaction does not cover pooled-money information.",
        "src/presentation/SemanticInputBoundary.cpp::semantic_open_money_management_tag/RealmzConsumeSemanticOpenMoneyManagementEvent"},
    ...
    {"dungeon.info.party_vitals", Surface::dungeon,
        Kind::essential_information, Status::semantic_complete,
        "The shared code-native party rail presents every member during dungeon map and first-person play with row 1 name, Lv, and AC; row 2 stamina plus SP for every caster selected solely by nonzero Classic spellpointsmax, including zero current SP, or ATK cadence for a noncaster; and row 3 state summary. Detached capture copies ac, spellpoints/max, normattacks, attackbonus, and conditions. Cadence follows Classic updatechar exactly: raw normattacks + attackbonus, Speedy condition 23 doubles first, then Slow condition 6 integer-halves. Adjusted values 0..19 render as reduced half fractions and every other value renders `> 10`. Each layout retains complete unelided semantic accessibility text for every row and state; the dungeon selection control carries it in accessibility-label metadata, but this slice makes no OS-publication claim. The rail remains read-only and introduces no action, tag, input, Classic-source, or replay-vocabulary change.",
        "src/presentation/PartyRailLayout.cpp::compute_party_rail_layout/layout_member"},
    ...
    {"dungeon.info.party_condition_indicators", Surface::dungeon,
        Kind::essential_information, Status::semantic_complete,
        "The shared code-native PARTY STATUS ribbon presents Classic's ordered party-wide effect indicators during dungeon map and first-person play. Detached capture copies exactly signed partycondition indices 1 through 8, excluding Torch index 0 and unused index 9; every nonzero value, including negative Search or equipment sentinels, is active. The model preserves Classic index order and labels Waterworld, Dragon Hide, Discover Secret, Wizard Eye, Search, Free Fall / Levitate, Sentry, and Charm Resistance without describing raw values as durations. Layout retains every complete token and unelided semantic accessibility string while using deterministic visible +N elision only when needed. This read-only information path makes no OS-publication claim and introduces no action, tag, input, Classic-source, or replay-vocabulary change.",
        "src/presentation/PartyRailLayout.cpp::compute_party_rail_layout"},
    ...
    {"dungeon.info.world_coordinates", Surface::dungeon,
        Kind::essential_information, Status::semantic_complete,
        "The shared responsive action-header world-context strip beside the persistent action-page tabs presents world-only dungeon coordinates during map and first-person play. It displays exact signed floorx and floory values without clamping. Any nonzero xydisplayflag instead conceals both coordinates as literal ? values with no raw-coordinate leakage. The strip is absent in combat and retains complete internal semantic text without an OS-publication claim. This read-only information path adds no action, tag, input, Classic-source, or replay-vocabulary change.",
        "src/presentation/WorldContextLayout.cpp::compute_world_context_layout"},
    {"dungeon.info.calendar_clock", Surface::dungeon,
        Kind::essential_information, Status::semantic_complete,
        "The shared responsive action-header world-context strip beside the persistent action-page tabs presents world-only campaign day and time during dungeon map and first-person play. Campaign day is displayed at Classic's signed-short boundary with no +1 and no calendar conversion. Time is a deterministic %I:%M %p-equivalent, zero-padded English 12-hour rendering derived from raw hour and minute with no locale or timezone normalization. The strip is absent in combat and retains complete internal semantic text without an OS-publication claim. This read-only information path adds no action, tag, input, Classic-source, or replay-vocabulary change.",
        "src/presentation/WorldContextLayout.cpp::compute_world_context_layout"},
    {"dungeon.info.fatigue", Surface::dungeon,
        Kind::essential_information, Status::semantic_complete,
        "The dungeon PARTY STATUS ribbon presents Classic's exact signed fatigue as FAT raw/135 with a bounded meter and explicit non-color band: baseline through 70, elevated from 71 through 105, and critical above 105, matching updatefat's two strict thresholds without normalizing the retained raw value. Fatigue remains absent from the combat ribbon because Classic returns before drawing it in combat. Complete internal semantic accessibility text is retained without claiming OS publication; no action, tag, input, Classic-source, or replay-vocabulary path is added.",
        "src/presentation/PartyRailLayout.cpp::compute_party_rail_layout"},
    ...
    {"dungeon.info.search_and_torch_state", Surface::dungeon,
        Kind::essential_information, Status::semantic_complete,
        "The shared responsive action-header world-context strip beside the persistent action-page tabs presents world-only Search from raw signed partycondition[5] and Torch from raw signed partycondition[0] during dungeon map and first-person play. Any nonzero value, including negative, is active; exact signed raw values are retained and displayed without calling either value a duration, turns, or charges. The usable Torch source remains action-availability data only. Both active and inactive states use explicit non-color markers. The strip is absent in combat and retains complete internal semantic text without an OS-publication claim. This read-only information path adds no action, tag, input, Classic-source, or replay-vocabulary change.",
        "src/presentation/WorldContextLayout.cpp::compute_world_context_layout"},
    ...
    {"combat.info.party_vitals", Surface::combat,
        Kind::essential_information, Status::semantic_complete,
        "The shared code-native party rail presents every member during combat with row 1 name, Lv, and AC; row 2 stamina plus SP for every caster selected solely by nonzero Classic spellpointsmax, including zero current SP, or ATK cadence for a noncaster; and row 3 state summary. Detached capture copies ac, spellpoints/max, normattacks, attackbonus, and conditions. Cadence follows Classic updatechar exactly: raw normattacks + attackbonus, Speedy condition 23 doubles first, then Slow condition 6 integer-halves. Adjusted values 0..19 render as reduced half fractions and every other value renders `> 10`. Each layout retains complete unelided semantic accessibility text for every row and state; combat has no selection control, and this slice makes no OS-publication claim. The rail remains read-only and introduces no action, tag, input, Classic-source, or replay-vocabulary change.",
        "src/presentation/PartyRailLayout.cpp::compute_party_rail_layout/layout_member"},
    ...
    {"combat.info.party_condition_indicators", Surface::combat,
        Kind::essential_information, Status::semantic_complete,
        "The shared code-native PARTY STATUS ribbon presents Classic's ordered party-wide effect indicators during combat. Detached capture copies exactly signed partycondition indices 1 through 8, excluding Torch index 0 and unused index 9; every nonzero value, including negative Search or equipment sentinels, is active. The model preserves Classic index order and labels Waterworld, Dragon Hide, Discover Secret, Wizard Eye, Search, Free Fall / Levitate, Sentry, and Charm Resistance without describing raw values as durations. Layout retains every complete token and unelided semantic accessibility string while using deterministic visible +N elision only when needed. Combat deliberately omits fatigue and pooled money, has no selection-control metadata for this ribbon, and this slice makes no OS-publication claim and introduces no action, tag, input, Classic-source, or replay-vocabulary change.",
        "src/presentation/PartyRailLayout.cpp::compute_party_rail_layout"},
